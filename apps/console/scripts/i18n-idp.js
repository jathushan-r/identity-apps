...
const metaPath = path.join(__dirname, "..", "src", "extensions", "i18n", "temp");
...
const metaFilePath = path.join(metaPath, metaJsonFileName);
...
for (const value of Object.values(i18nExtensions)) {
    if (!value || !value.name || !value.idp) {
        continue;
    }

    const fileContent = JSON.stringify(value.idp, undefined, 4);
    const hash = crypto
        .createHash("sha1")
        .update(JSON.stringify(fileContent))
        .digest("hex");
    const fileName = `idp.${hash.substr(0, 8)}.json`;
    ...
    meta[value.name].paths.idp = meta[value.name].paths.idp.replace("{hash}", hash.substr(0, 8));
...
// const tmpDir = path.join(__dirname, "..", "src", "features", "identity-providers", "i18n", "tmp");
